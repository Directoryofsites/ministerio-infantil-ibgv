...
const Dashboard = ({ clases, onSelectClase, onNavigate, onNewClass, onEditObservations, isAdmin, onLoginClick, onLogout }) => {
    ...
    const availablePeriods = Array.from(new Set(clases.map(clase => {
    ...
    const filteredClases = selectedPeriod === 'all'
        ? clases
        : clases.filter(clase => {
            const date = new Date(clase.fecha);
            ...
            <main className="flex-1 p-5 pb-28 space-y-7">
                ...
                <section className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
                    <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-2">
                            <span className="material-symbols-outlined text-primary !text-lg notranslate">calendar_month</span>
                            <span className="text-xs font-bold text-charcoal/60 uppercase tracking-wider">Filtrar por Periodo:</span>
                        </div>
                        <select 
                            value={selectedPeriod}
                            onChange={(e) => setSelectedPeriod(e.target.value)}
                            className="flex-1 bg-bone border border-gray-100 text-sm font-bold text-charcoal px-3 py-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 appearance-none text-center capitalize"
                        >
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        ...
                        {filteredClases.length === 0 && (
                            <div className="text-center p-8 bg-white rounded-2xl border border-gray-100 shadow-soft">
                                <p className="text-silver text-sm">No hay clases programadas para este periodo.</p>
                            </div>
                        )}
                    </div>
                </section>
